...
- **The ship tier shown for review is the one the build was actually sized
  from.** The row used to repeat the tier read off the badge, even when a
  correction made earlier or the slots visible on screen had already
  overruled it — so a screenshot whose badge read `T1` under a confirmed
  `T6` offered `T1` as certain, and accepting it put `T1` back into the
  training data. The row now shows the value in use, and its confidence says
  where that value came from: a confirmation counts as certain, a badge
  reading is graded, and a tier worked out from the slots always waits for
  review.
- **Ship names survive a misread letter.** A dropped or swapped character
  used to hand the read to a looser match and could return a different ship
  entirely — a *Lexington* becoming a *Universe*. Recognition now prefers the
  ship whose name really matches and falls back to the class only when
  nothing does, and a guessed class is no longer reported as certain.
- **A tier badge split across two fragments is read again.** On smaller
...
- **A tier read too low is corrected from the picture.** `[T6-X2]` misread as
  `[T6-X]` left every panel a slot short with nothing to explain why. When
  the screenshot holds more slots than the badge allows, the picture wins.
- **Empty and locked slots are no longer learned as items.** Two faults, one
  in each direction. A blank picture confirmed as "empty" matched every icon
  it was compared against, so recognition had to argue itself out of calling
  every slot empty. And a picture of a blank slot filed under an item's name
  taught it that "nothing" *is* that item — which is why one bridge officer
  ability kept turning up on empty seats. Neither is accepted for learning
  now, and auto-accept leaves both cases for review.
...
- **Right-clicking an item opens the wiki page it was catalogued from.** Most
  ...
  on the page of its set — so the link used to land on "page does not exist"
  for most traits and abilities.
- **The review list no longer jumps to the top mid-scroll**, and **Mark Done
  reappears correctly after a row is deleted**.
...
- **Gear the game draws differently in the 23rd century is recognised as the
  ...
  actually matched.
- **A record of what recognition decided** is kept per screenshot, so a
  change in quality can be seen rather than guessed at.

### Changed
- **Community pictures are stored in subfolders.** The shared collection
  outgrew a single directory. The local copy reorganises itself the first
  time it syncs — nothing is downloaded again — and an older version keeps
