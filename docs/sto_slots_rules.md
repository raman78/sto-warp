...
generated by `dev/diag_boff_marker_swatch.py` from the same HSV
centers used by the detector. `dev/` is the maintainer's local working
set — gitignored and absent from a checkout — so that path is a
reproduction pointer; the committed image is the usable artefact.
