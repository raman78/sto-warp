...
|---|---|---|
| `_merge_recognition` (`trainer_window.py`) | fresh items vs rows already on screen | Auto-Detect is "find what is not here yet", so a new item that collides with a visible row is dropped |
| `_populate_review_panel` (`trainer_window.py`) | fresh items vs `annotations.json` | a confirmed annotation carries the user's name/state and must survive re-detection |
...
chars (`Annotation._make_id` in `training_data.py`) — bbox and slot, not name. Matching walks three
steps, in order:

1. **Exact `ann_id`.** Same bbox, same slot.
2. **Bbox IoU ≥ `IOU_RECOVER` (0.5), any slot** — `_find_legacy_aid`
   (`WarpCoreWindow._populate_review_panel` in `trainer_window.py`). Absorbs detector drift of a few pixels and slot
...
`pending` (`trainer_window.py`ff) so the user re-verifies rather than
having either side silently swallowed.

Confirmed annotations that no fresh item matched are appended after the loop
(`WarpCoreWindow._populate_review_panel` in `trainer_window.py`) — this is what keeps 31 confirmed equipment rows on
...
(`_single_instance_twin`, called from `WarpCoreWindow._populate_review_panel`). The fold
reuses the normal match path, so a name disagreement still surfaces as a
conflict. A second fresh item for the same slot, and a second confirmed entry from
legacy data, are both dropped — both guards live in
`_populate_review_panel`.

`SINGLE_INSTANCE_SLOTS` (`training_data.py`) is the membership list. It
...
(0.3, `_MERGE_IOU_OVERLAP` in `trainer_window.py`) is dropped; the confirmed row is re-added by the
...
`_confirmed_overlap` in `trainer_window.py`): `Ship Type` spans the class line and `Ship Tier` the
...
(`TrainingDataManager.add_annotation` in `training_data.py`) drops any other confirmed annotation for a
...
|---|---|---|
| `order_for_review groups: … Ship Tier(2) …` | fresh tier bbox and confirmed tier bbox overlap below 0.5, so step 2 missed them — the case rule 1 closes | log line `folded onto confirmed bbox=` |
| Two rows on one icon, one confirmed and one pending | IoU between 0.3 and 0.5 — matched by neither step 2 nor rule 2 before it existed | log line `overlaps confirmed` |
| A confirmed row silently replaced by a fresh name | equivalence class or a previously rejected community proposal | `WarpCoreWindow._populate_review_panel` |
| Confirmed rows vanish after Auto-Detect | item filtered out of `preserve_existing` by `_is_legacy` (no `slot_index`) **and** not present on disk | `WarpCoreWindow._start_recognition` |

## Decisions and trade-offs

**The confirmed bbox wins, not the fresh one.** Rule 1 could have adopted
today's geometry the way `_find_legacy_aid` does. It does not: the
...
Side effect: positional slot suggestion (`WarpCoreWindow._suggest_slot_from_position` in `trainer_window.py`) now skips
